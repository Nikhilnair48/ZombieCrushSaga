...
public class ZombieCrushSagaTile extends Sprite
{
    ...
    public boolean match(ZombieCrushSagaTile testTile) {
        String ID = testTile.getSpriteType().getSpriteTypeID();
        int ID2 = specialToRegular(ID);
        int sT_ID = specialToRegular(spriteType.getSpriteTypeID());

        // C TYPE TILES ARE ONLY MATCHES IF THEY HAVE THE SAME IMAGE (I.E. THEY'RE THE SAME SPRITE TYPE)
        if (spriteType.getSpriteTypeID().equals(ID) || sT_ID == ID2 - 6 || sT_ID == ID2 - 13 || ID2 == sT_ID - 6 || ID2 == sT_ID - 13) {
            return true;
        } else // THEY DIFFER IN SOME CRITICAL WAY SO THEY ARE NOT A MATCH
        {
            return false;
        }
    }
    
    public boolean gridMatch(ZombieCrushSagaTile testTile) {
   
        String ID = testTile.getSpriteType().getSpriteTypeID();
        
        if (spriteType.getSpriteTypeID().equals(ID))
            return true;
        else
            return false;
    }
